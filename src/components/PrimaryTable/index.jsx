  ...
  const columns = [
    {
      ...
      render: (_, record) => {
        return (
          <div className="flex">
            <img src={`${record?.packedImage}`} className="rounded" width="48px" height="48px" />
            <div className="flex flex-col ml-[12px!important] justify-between">
              <div className="font-normal text-base">{record?.shippingAddress?.name}</div>
              <div className="flex">
                ...
                {record?.firstOrder&&<span className="font-bold text-[10px] leading-[100%] tracking-[0%] gap-2.5 rounded p-[3px] bg-[#707EFA] text-[white]">
                  New
                </span>}
              </div>
            </div>
    ...
    },
    {
      ...
      render:(_,record)=>{
        return(
          <span>{record?.lineItems?.length}</span>
        )
      }
    },
    {
